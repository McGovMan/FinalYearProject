...
type Backends *struct {
	IPAddress    string `json:"ip"`
	SourceIPAddress    string `json:"source_ip"`
...
type Forwarding struct {
	PassInterfaces []string `json:"pass_interfaces"`
 ...
 * and the server IP, this creates a 32 byte long weight. We pick the largest three
...
func (app *Application) syncForwardingMap(backends []Backends, index uint32, seed string) (error) {
	...
	for x := uint32(0); x < 65536; x++ {
		...
		ips := make([]uint32, app.Config.NumberOfCollumns+1)
		
		ips[0] = Pack32BinaryIP4(backends[bestBackends[0].BackendsArrayIndex].SourceIPAddress)
		for bb := uint32(0); bb < uint32(len(bestBackends)); bb++ {
			ips[bb+1] = Pack32BinaryIP4(backends[bestBackends[bb].BackendsArrayIndex].IPAddress)
		}

		assertExit(newMap.Put(x, ips))
	}
...
func main() {
	...
	// we want the row to include x number of backend IPs plus
	// the source IP the load balancer should be using for sending
	// out packets on
	...
	// Pin the packet_stats map to a file descriptor so it can be read by the stats aggregator kernel module
	packetStatsMap := app.Collection.Maps["packet_stats"]
	if packetStatsMap == nil {
		fmt.Errorf("packet_stats map not found")
	}
	packetStatsMap.Pin("/sys/fs/bpf/packet_stats");
	...
	defer (func() { packetStatsMap.Unpin() })()

	for _, interfaceName := range app.ForwardingFile.PassInterfaces {
		dummyLink, err := netlink.LinkByName(interfaceName)
		assertExit(err)
		...
		b := make([]byte, 4)
		binary.LittleEndian.PutUint32(b, uint32(dummyLink.Attrs().Index))
		assertExit(txPortsMap.Put(binary.LittleEndian.Uint32(b), binary.LittleEndian.Uint32(b)))
	}
