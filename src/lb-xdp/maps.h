...
/*
 * This struct contains all the statistics related to packet handling
 */
typedef struct {
    __u64 total_packets;
    __u64 total_packets_dropped;
    __u64 total_packets_accepted;
    __u64 total_packets_forwarded;
    __u64 bytes_received;
} packet_stats_row;

/*
 * The packet_stats_map contains a per CPU allocated array of size of one.
 * The array contains one row which is a struct that contains all packet statistics.
 */
struct bpf_map_def SEC("maps") packet_stats = {
        .type        = BPF_MAP_TYPE_PERCPU_ARRAY,
        .key_size    = sizeof(__u32),
        .value_size  = sizeof(packet_stats_row),
        .max_entries = 1,
};
